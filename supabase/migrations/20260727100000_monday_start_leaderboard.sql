-- KO-186: make the weekly leaderboard boundary explicit and timezone-safe.
-- PostgreSQL DATE_TRUNC('week', ...) is Monday-starting, but the input must be
-- derived from the Jakarta-local date rather than the database session date.
...
BEGIN
  ...
  ranked_xp AS (
    SELECT eu.id AS user_id, eu.display_name,
    ...
    FROM eligible_users eu LEFT JOIN xp_totals xt ON xt.user_id = eu.id
  ),
  kp_totals AS (
  ...
  ranked_kp AS (
    ...
    FROM eligible_users eu LEFT JOIN kp_totals kt ON kt.user_id = eu.id
  )
  SELECT
    COALESCE((SELECT jsonb_agg(jsonb_build_object(
      'rank', rx.rank, 'user_id', rx.user_id, 'display_name', rx.display_name,
      'xp_this_week', rx.xp_this_week, 'is_current_user', rx.user_id = p_user_id
    ) ORDER BY rx.rank) FROM ranked_xp rx WHERE rx.rank <= 10), '[]'::jsonb),
    COALESCE((SELECT jsonb_agg(jsonb_build_object(
      'rank', rk.rank, 'user_id', rk.user_id, 'display_name', rk.display_name,
      'koin_points_this_week', rk.kp_this_week, 'is_current_user', rk.user_id = p_user_id
    ) ORDER BY rk.rank) FROM ranked_kp rk WHERE rk.rank <= 10), '[]'::jsonb)
  INTO v_xp_json, v_kp_json;

  RETURN jsonb_build_object('week_start', v_week_start, 'xp', v_xp_json, 'koin_points', v_kp_json);
END;
$$;
