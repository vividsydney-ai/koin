...
-- ==================== MONEY BASICS ====================
SELECT
  (SELECT id FROM lessons WHERE slug = 'money-basics-101')::uuid AS lesson_id,
  'example'::text AS variant_type,
  jsonb_build_object(
  ...
  'money_basics'::text AS topic_tag
UNION ALL
SELECT
  (SELECT id FROM lessons WHERE slug = 'money-basics-101')::uuid AS lesson_id,
  'question'::text AS variant_type,
  jsonb_build_object(
...
UNION ALL
SELECT
  (SELECT id FROM lessons WHERE slug = 'budgeting-101')::uuid AS lesson_id,
  'example'::text AS variant_type,
  jsonb_build_object(
...
UNION ALL
SELECT
  (SELECT id FROM lessons WHERE slug = 'inflation-101')::uuid AS lesson_id,
  'example'::text AS variant_type,
  jsonb_build_object(
...
UNION ALL
SELECT
  (SELECT id FROM lessons WHERE slug = 'risk-return-101')::uuid AS lesson_id,
  'example'::text AS variant_type,
  jsonb_build_object(
...
UNION ALL
SELECT
  (SELECT id FROM lessons WHERE slug = 'idx-basics-101')::uuid AS lesson_id,
  'example'::text AS variant_type,
  jsonb_build_object(
...
SELECT lesson_id, variant_type, body, difficulty, topic_tag
FROM variants
