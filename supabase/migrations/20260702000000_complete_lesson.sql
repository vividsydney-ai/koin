-- Migration 015: Atomic lesson completion RPC
-- Single function that records a lesson attempt, updates progress, awards XP,
-- updates daily check-in, maintains streak, updates mastery, and awards badges.
-- Runs as SECURITY DEFINER so it can write across user-private tables in one
-- transaction, but it validates auth.uid() = p_user_id before doing anything.
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    v_last_completed := v_today;
    INSERT INTO streaks (user_id, current_streak_days, longest_streak_days, last_completed_on, streak_status)
    VALUES (p_user_id, 1, 1, v_today, 'active');
    INSERT INTO streak_events (user_id, event_type, streak_days_at_event)
    VALUES (p_user_id, 'preserved', 1);
  ELSE
    v_last_completed := v_streak.last_completed_on;

    IF v_last_completed = v_today THEN
      v_new_streak := v_streak.current_streak_days;
      v_streak_status := v_streak.streak_status;
    ELSIF v_last_completed = v_today - INTERVAL '1 day' THEN
      ...
      INSERT INTO streak_events (user_id, event_type, streak_days_at_event)
      VALUES (p_user_id, 'preserved', v_new_streak);
    ELSE
      -- Streak broken.
      INSERT INTO streak_events (user_id, event_type, streak_days_at_event)
      VALUES (p_user_id, 'broken', v_streak.current_streak_days);
      v_new_streak := 1;
      v_streak_status := 'active';
      UPDATE streaks
  ...
  -- Streak milestones.
  IF v_new_streak IN (3, 7) THEN
    INSERT INTO streak_events (user_id, event_type, streak_days_at_event)
    VALUES (p_user_id, 'milestone', v_new_streak);
  END IF;
