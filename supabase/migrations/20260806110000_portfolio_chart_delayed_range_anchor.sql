-- KO-274: delayed EOD ranges should anchor to the latest available close,
-- not the wall-clock date, so valid delayed movement remains visible.
...
BEGIN
  ...
  SELECT MAX(md.trade_date) INTO v_reference_date
  ...
  v_window_days := CASE WHEN p_days = 1 THEN 2 ELSE p_days END;

  SELECT GREATEST(
    p.created_at::date,
    COALESCE(v_reference_date - (v_window_days - 1), p.created_at::date)
  )
  INTO v_start_date
  FROM public.portfolios AS p
  ...
  WITH candidate_dates AS (
    ...
    WHERE s.portfolio_id = v_portfolio_id
      ...
      AND s.snapshot_date <= v_reference_date
    ...
    WHERE md.trade_date >= v_start_date
      AND md.trade_date <= v_reference_date
      AND EXISTS (
        SELECT 1
        FROM public.trades AS t
        WHERE t.portfolio_id = v_portfolio_id
    ...
    SELECT v_reference_date
    WHERE v_reference_date >= v_start_date
  ),
...
COMMENT ON FUNCTION public.get_portfolio_value_history(UUID, INTEGER) IS
  'Returns owner-scoped mark-to-market paper portfolio history using available delayed/simulated closes, anchored to the latest available close.';
