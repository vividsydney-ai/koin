-- KO-210: a lesson check must be answered correctly before completion.
-- Wrong checks are recorded as incomplete practice attempts only: they award
-- no XP/Koin Points, do not advance mastery, and do not open the next lesson.
...
BEGIN
  ...
  -- A failed check is an incomplete attempt. It must never create progress,
  -- award XP/Koin Points, update streaks, or unlock the next lesson.
  IF NOT p_quiz_correct THEN
    INSERT INTO lesson_attempts (
      ...
      FALSE, NULL, p_answers_json, p_time_spent_seconds
    );
