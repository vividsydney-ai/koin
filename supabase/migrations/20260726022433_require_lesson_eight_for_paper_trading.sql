-- KO-160: Paper trading becomes available only after the Lesson 8 milestone
-- and the existing trade onboarding are complete. This must live in the RPC
-- because the client-side Trade lock can otherwise be bypassed with a direct RPC call.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Complete Lesson 8 before paper trading.';
  END IF;
