-- Migration 052: Reduce the first-completion minimum-time gate from 30s to 10s
-- and add a specific error message so the UI can explain why a fast finish is
-- rejected. Replays are still exempt because they earn 0 XP.
...
BEGIN
  ...
  -- Minimum time gate: first completions only, lowered to 10 seconds.
  -- Replays earn 0 XP and are intentionally allowed to be fast.
