-- Migration KO-QUIZ-001: Make complete_lesson return the next uncompleted lesson
--
-- After a curriculum renumbering some learners already have intermediate lessons
-- marked as completed. The "Next lesson" button was jumping over those completed
-- lessons to a much later one (e.g. from Budgeting straight to Emergency Fund).
-- This change makes the next-lesson picker skip lessons the user has already
-- completed, landing on the first unpublished lesson in curriculum order.
-- If every following lesson is completed, it falls back to the immediate next
-- published lesson so the button never stays blank for no reason.
...
BEGIN
  ...
  -- Next lesson: first following lesson the user has NOT completed.
  SELECT slug INTO v_next_lesson_slug
  FROM lessons l
  WHERE l.lesson_number > v_lesson.lesson_number
  ...
  -- Fallback: if every following lesson is already completed, still point to
  -- the immediate next published lesson so the UI never loses the CTA.
  IF v_next_lesson_slug IS NULL THEN
    SELECT slug INTO v_next_lesson_slug
    FROM lessons
