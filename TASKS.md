...
- [x] UI-LIFT-1 — Bottom tab bar: SVG icons + gradient active state (app/(app)/layout.tsx, new GradientIcon; keep 6 tabs) [S]
- [x] UI-LIFT-2 — Home/profile stat cards + streak/XP/portfolio number treatment; shared <StatCard> reused by profile + LessonPlayer completion tiles [M]
- [x] UI-LIFT-3 — Leaderboard pattern on Friends page (avatar chips, is-you row; hide rank-change arrow when no delta data — no migration) [M]
- [x] UI-LIFT-4 — Price chart card on Trade page (synthesized candles from daily closes; no migration) [L/S]
- [x] UI-LIFT-5 — Library filter chips (replace selects) + profile badge grid [S]
