...
async function main() {
  ...
  const [{ data: blocks, error: blockError }, { data: variants, error: variantError }, { data: recalls, error: recallError }, { data: links, error: linkError }] = await Promise.all([
    supabase.from("lesson_visual_blocks").select("lesson_id, block_type, display_order, data_status, content, is_published").in("lesson_id", lessonIds).eq("is_published", true),
